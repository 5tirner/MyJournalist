...
def signup(req):
    ...
    if serial.is_valid():
        ...
        if (password_status != 'Strong'):
            return response.Response({'password': password_status},
        ...
        try:
            ...
            send_verification_email(code, serial.validated_data['email'])
...
def profile_data(req):
    ...
    isAvailable = False
    try:
        userFields.objects.get(identity=user_data.identity)
        isAvailable = True
    except:
        pass
    return response.Response({'Fields': isAvailable},
                             status=status.HTTP_200_OK)
